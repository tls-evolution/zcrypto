...
func pickSignatureAlgorithm(pubkey crypto.PublicKey, peerSigAlgs, ourSigAlgs []SignatureScheme, tlsVersion uint16) (SignatureScheme, uint8, crypto.Hash, error) {
	if tlsVersion < VersionTLS12 || len(peerSigAlgs) == 0 {
		...
		case *rsa.PublicKey:
			if tlsVersion < VersionTLS12 {
				return 0, signature13_PKCS1v15, crypto.MD5SHA1, nil
			} else {
				return PKCS1WithSHA1, signature13_PKCS1v15, crypto.SHA1, nil
			}
		case *ecdsa.PublicKey:
			return ECDSAWithSHA1, signature13_ECDSA, crypto.SHA1, nil
		default:
	...
	for _, sigAlg := range peerSigAlgs {
		...
		if (sigType == signature13_PKCS1v15 || hashAlg == crypto.SHA1) && tlsVersion >= VersionTLS13 {
		...
		switch pubkey.(type) {
		case *rsa.PublicKey:
			if sigType == signature13_PKCS1v15 || sigType == signature13_RSAPSS {
