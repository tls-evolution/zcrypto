...
func initDefaultCipherSuites() {
	...
	// TODO check as not in the original diff
	varDefaultTLS13CipherSuites = make([]uint16, len(stdlibTLS13CipherSuites))
	for i, suite := range stdlibTLS13CipherSuites {
		varDefaultTLS13CipherSuites[i] = suite.id
	}
}

// TODO: original TLS 1.3 diff for this function
//
// func initDefaultCipherSuites() {
//         var [-topCipherSuites-]{+topCipherSuites, topTLS13CipherSuites+} []uint16
...
type tls13notImplementedAbortErrorT struct {
	err error // wrapped error
}

func (e *tls13notImplementedAbortErrorT) Error() string {
	return fmt.Sprintf("%v", e.err)
}

func tls13notImplementedAbortError(err error) error {
	res := tls13notImplementedAbortErrorT{}
	res.err = err
	return &res
}

func IsTLS13notImplementedAbortError(e error) bool {
	_, ok := e.(*tls13notImplementedAbortErrorT)
	return ok
}
